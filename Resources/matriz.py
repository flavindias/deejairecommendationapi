...
# querrys sql
roomUsersQuerry = (
    "SELECT rooms_users.user_id FROM rooms_users WHERE room_id = %s" % room)
userTracksQuerry = (
    "SELECT users_tracks.user_id, users_tracks.track_id FROM users_tracks, (%s) roomUsers WHERE roomUsers.user_id = users_tracks.user_id" % roomUsersQuerry)
tracksQuerry = (
    "SELECT DISTINCT features.* FROM features, (%s) usertracks WHERE usertracks.track_id = features.track_id" % userTracksQuerry)
